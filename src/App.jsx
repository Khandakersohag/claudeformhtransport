...
const SCRIPT_URL = import.meta.env.VITE_SCRIPT_URL || "https://script.google.com/macros/s/AKfycbyPZtyFUb3jH4czBoGryDu-1C3EGlPnEXh4UjPQ5_v7xn_PXg5LlhSRsSWBx3lyTYs-/exec";
...
const GS = {
  ...
  // Special POST for Users — needs response confirmation
  userPost: async (body) => {
    try {
      // Use form-encoded to avoid CORS preflight
      const params = new URLSearchParams();
      params.append("data", JSON.stringify(body));
      const res = await fetch(SCRIPT_URL + "?action=userPost", {
        method:"POST",
        mode:"no-cors",
      ...
      console.error("userPost error:", e);
      return false;
    }
  },
  getAll: async (sheet) => {
...
const getUsers = () => {
  const stored = LS.get("mht_users", null);
  // If localStorage has users, use them (they are synced from Sheets on load)
...
const saveUsers = (u) => {
  ...
  LS.set("mht_users", u);
  // Then sync to Sheets in background (non-blocking)
  u.forEach(user => {
    GS.post({
      action:"save",
      sheet:"Users",
      row:{
        ...
        active: String(user.active===true||user.active==="true"),
      }
    });
  });
};
...
function LoginPage({onLogin}){
  ...
  const recover=()=>{
    ...
    const user=users.find(u=>u.username===recUsername);
    if(!user){setRecMsg("এই ইউজারনেম পাওয়া যায়নি");return;}
    const updated=users.map(u=>u.username===recUsername.trim()?{...u,password:recNewPass.trim()}:u);
    saveUsers(updated);
...
export default function App(){
  ...
  const[currentUser,setCurrentUser]=useState(()=>{
    const session=LS.get("mht_session",null);
      ...
      try {
        ...
        if(sheetData){
          setData(prev=>{
            ...
            // Always sync users from Sheets to localStorage (overwrite local)
            if(sheetData.users&&sheetData.users.length>0){
              LS.set("mht_users", sheetData.users);
              console.log("Users synced from Sheets:", sheetData.users.length);
            }
            return merged;
          });
  ...
  const handleLogout=()=>{LS.set("mht_session",null);setCurrentUser(null);setPage("dashboard");};
      ...
      <div style={{display:"flex",minHeight:"100vh",background:"var(--navy-950)"}}>
        ...
        <aside style={{position:"fixed",top:0,left:0,height:"100%",width:230,zIndex:50,background:"linear-gradient(180deg,var(--navy-900) 0%,var(--navy-950) 100%)",borderRight:"1px solid rgba(100,255,218,0.07)",display:"flex",flexDirection:"column",transform:sidebarOpen?"translateX(0)":"translateX(-100%)",transition:"transform 0.28s cubic-bezier(0.4,0,0.2,1)",boxShadow:sidebarOpen?"4px 0 40px rgba(2,12,27,0.8)":"none"}}>
          ...
          <div style={{padding:"14px 20px",borderTop:"1px solid rgba(100,255,218,0.06)"}}>
            <p style={{fontSize:11,color:"var(--text-muted)",textAlign:"center"}}>v14.0 · User System Fixed ✅</p>
          </div>
        </aside>
