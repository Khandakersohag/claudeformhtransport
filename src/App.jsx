...
const saveUsers = async (u) => {
  LS.set("mht_users", u);
  // Save each user to Sheets (upsert by id)
  for(const user of u){
    await GS.post({
      action:"save",
      sheet:"Users",
...
function LoginPage({onLogin}){
  ...
  const login=()=>{
    ...
    setLoading(true);
    // Always fetch fresh users from Sheets before login
    GS.getAll("Users").then(sheetUsers=>{
      let users;
      if(sheetUsers&&sheetUsers.length>0){
        // Parse and save fresh from Sheets
      ...
      if(user){
        ...
        setError("ইউজারনেম বা পাসওয়ার্ড ভুল! (অফলাইন মোড)");
      }
      setLoading(false);
    });
  };
  ...
  const recover=()=>{
    ...
    const updated=users.map(u=>u.username===recUsername?{...u,password:recNewPass}:u);
    LS.set("mht_users", updated);
    saveUsers(updated);
      ...
      <div style={{display:"flex",minHeight:"100vh",background:"var(--navy-950)"}}>
        ...
        <aside style={{position:"fixed",top:0,left:0,height:"100%",width:230,zIndex:50,background:"linear-gradient(180deg,var(--navy-900) 0%,var(--navy-950) 100%)",borderRight:"1px solid rgba(100,255,218,0.07)",display:"flex",flexDirection:"column",transform:sidebarOpen?"translateX(0)":"translateX(-100%)",transition:"transform 0.28s cubic-bezier(0.4,0,0.2,1)",boxShadow:sidebarOpen?"4px 0 40px rgba(2,12,27,0.8)":"none"}}>
          ...
          <div style={{padding:"14px 20px",borderTop:"1px solid rgba(100,255,218,0.06)"}}>
            <p style={{fontSize:11,color:"var(--text-muted)",textAlign:"center"}}>v13.0 · User Sync Fixed ✅</p>
          </div>
        </aside>
