...
import { ROLES, ROLE_PERMS, ALL_PAGES, getUserPages, getUsers, getCurrentUser, logoutUser, syncUsersFromSheets } from "./utils/auth.js";
...
export default function App() {
  ...
  const allowedPages = getUserPages(currentUser); // Dynamic per-user pages
  ...
  const pages = Object.fromEntries(
    Object.entries(allPages).filter(([k]) => {
      if (k === "users") return perm.canManageUsers;
      if (allowedPages.includes("all")) return true; // Admin
      return allowedPages.includes(k);
    })
  );
      ...
      <div style={{display:"flex",minHeight:"100vh",background:"var(--navy-950)"}}>
        ...
        <aside style={{position:"fixed",top:0,left:0,height:"100%",width:230,zIndex:50,
          ...
          <div style={{padding:"14px 20px",borderTop:"1px solid rgba(100,255,218,0.06)"}}>
            <p style={{fontSize:11,color:"var(--text-muted)",textAlign:"center"}}>v18.0 · Dynamic Permissions ✅</p>
          </div>
        </aside>
