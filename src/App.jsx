...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbyPZtyFUb3jH4czBoGryDu-1C3EGlPnEXh4UjPQ5_v7xn_PXg5LlhSRsSWBx3lyTYs-/exec";
  ...
  loadAllData: async () => {
    try {
      ...
      const parsedSalaries = salaries.map(s => ({
        ...
        paidDate: s.paidDate ? new Date(s.paidDate).getTime() : Date.now(),
        startDate: s.startDate ? new Date(s.startDate).getTime() : Date.now(),
        endDate: s.endDate ? new Date(s.endDate).getTime() : Date.now(),
      })).filter(s=>s.id);
      ...
      const parsedTasks = tasks.map(t => ({
        ...t,
        id: String(t.id),
        dueDate: t.dueDate ? new Date(t.dueDate).getTime() : Date.now(),
      })).filter(t=>t.id);
      ...
      const parsedUsers = users.length > 0 ? users.map(u => ({
        ...u,
        id: String(u.id),
        active: u.active==="true"||u.active===true||u.active===undefined,
...
const getUsers = () => LS.get("mht_users", DEFAULT_USERS);
const saveUsers = (u) => {
  LS.set("mht_users", u);
  // Sync users to Sheets
  u.forEach(user => {
    GS.post({action:"save",sheet:"Users",row:{id:user.id,username:user.username,password:user.password,name:user.name,role:user.role,active:user.active}});
  });
};
...
function UserManagement({currentUser}){
  ...
  const save=()=>{
    ...
    GS.post({action:"save",sheet:"Users",row:{id:editId||Date.now(),name:uName,username:uUsername,role:uRole}});
    reset();
  };
      ...
      try {
        ...
        if(sheetData){
          setData(prev=>{
            ...
            // Update users if from Sheets
            if(sheetData.users&&sheetData.users.length>0){
              saveUsers(sheetData.users);
            }
            return merged;
          });
      ...
      <div style={{display:"flex",minHeight:"100vh",background:"var(--navy-950)"}}>
        ...
        <aside style={{position:"fixed",top:0,left:0,height:"100%",width:230,zIndex:50,background:"linear-gradient(180deg,var(--navy-900) 0%,var(--navy-950) 100%)",borderRight:"1px solid rgba(100,255,218,0.07)",display:"flex",flexDirection:"column",transform:sidebarOpen?"translateX(0)":"translateX(-100%)",transition:"transform 0.28s cubic-bezier(0.4,0,0.2,1)",boxShadow:sidebarOpen?"4px 0 40px rgba(2,12,27,0.8)":"none"}}>
          ...
          <div style={{padding:"14px 20px",borderTop:"1px solid rgba(100,255,218,0.06)"}}>
            <p style={{fontSize:11,color:"var(--text-muted)",textAlign:"center"}}>v10.0 · Sheets Sync ✅</p>
          </div>
        </aside>
