...
// FONT_LINK moved into App useEffect to avoid SSR crash and memory leak
...
const GS = {
  // NOTE: no-cors is required for Google Apps Script POST without custom CORS headers.
  // To enable full error detection, update your Apps Script to return proper CORS headers
  // and then switch mode to "cors" below.
  post: async (body) => {
    try {
      ...
      return true;
    } catch(e) { console.warn("GS.post failed (offline?):", e.message); return false; }
  },
  ...
  getAll: async (sheet) => {
    ...
    } catch(e) { console.warn(`GS.getAll(${sheet}) failed:`, e.message); return []; }
  },
  loadAllData: async () => {
    try {
...
const todayISO=()=>{const d=new Date();d.setMinutes(d.getMinutes()-d.getTimezoneOffset());return d.toISOString().split("T")[0];};
...
function DailyEntry({data,setData}){
  ...
  const addExp=()=>{const num=Number(expAmt);if(!expAmt||isNaN(num)||num<=0||num>10000000)return;setExpenses(p=>[...p,{category:expCat,amount:num,description:expDesc.trim()||expCat}]);setExpAmt("");setExpDesc("");};
  const save=()=>{
    const rec={id:editId||(crypto.randomUUID?crypto.randomUUID():Date.now().toString()),date:tsFrom(date),carCount:Number(carCount),tripCount:Number(tripCount),expenses,totalIncome,totalExpense,netProfit,createdAt:Date.now()};
...
function StaffSalary({data,setData}){
  ...
  const addStaff=()=>{
    ...
    const s={id:(crypto.randomUUID?crypto.randomUUID():Date.now().toString()),name:sName,position:sPos,dailyRate:Number(sRate),isActive:true};
  ...
  const pay=()=>{
    ...
    const sal={id:(crypto.randomUUID?crypto.randomUUID():Date.now().toString()),staffId:sel.id,staffName:sel.name,startDate:tsFrom(salStart),endDate:tsFrom(salEnd),totalDays:days,actualSalary:actual,paidAmount:paid,dueAmount:actual-paid,paidDate:Date.now()};
...
function ExpenseTracker({data,setData}){
  ...
  const add=()=>{
    ...
    const e={id:(crypto.randomUUID?crypto.randomUUID():Date.now().toString()),date:tsFrom(date),category:cat,amount:Number(amount),description:desc||cat,isRecurring:isRec};
...
function TaskManagement({data,setData}){
  ...
  const add=()=>{
    if(!title)return;
    const t={id:(crypto.randomUUID?crypto.randomUUID():Date.now().toString()),title,description:desc,status:"pending",priority,dueDate:tsFrom(dueDate),createdAt:Date.now(),assignedTo};
...
function VehicleManagement({data,setData}){
  ...
  const save=()=>{
    if(!vNum)return;
    const v={id:editId||(crypto.randomUUID?crypto.randomUUID():Date.now().toString()),number:vNum,model:vModel,type:vType,taxExp,fitExp,insExp,routeExp,lastService,createdAt:Date.now()};
...
function QuickEntry({data,setData,setPage,setSidebarOpen}){
  ...
  const save=()=>{
    ...
    if(existing){
      ...
      const rec={id:(crypto.randomUUID?crypto.randomUUID():Date.now().toString()),date:new Date(today).getTime(),carCount:Number(carCount),tripCount:Number(tripCount),expenses:[],totalIncome,totalExpense:0,netProfit:totalIncome,note,createdAt:Date.now()};
...
function UserManagement({currentUser}){
  ...
  const save=()=>{
    ...
    if(editId){
      ...
      const newUser={id:(crypto.randomUUID?crypto.randomUUID():Date.now().toString()),username:uUsername,password:uPass,name:uName,role:uRole,active:true};
      list=[...users,newUser];
    }
    saveUsers(list);
    setUsers([...list]);
...
export default function App(){
  ...
  // Inject Google Font safely (avoids SSR crash + cleans up on unmount)
  useEffect(()=>{
